...
class Residual(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1):
        ...
        self.conv1 = nn.Conv2d(in_channels,out_channels,kernel_size=(3,1),padding = (1,0),stride=stride)
        self.conv2 = nn.Conv2d(out_channels,out_channels,kernel_size=(3,1),padding = (1,0))
        self.conv3 = nn.Conv2d(in_channels,out_channels,kernel_size=(3,1),padding = (1,0),stride=stride)
...
class metal_net(nn.Module):
    def __init__(self,layers):
        ...
        self.conv1 =nn.Conv2d(1,64,kernel_size=(3,20),padding = (1,0),stride=1)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)

        self.layer1 = self.resnet_block(64, 64, layers[0])
        self.layer2 = self.resnet_block(64, 128, layers[1])
        self.layer3 = self.last_block(128,1)
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x= self.layer3(x)
        softmax_output = self.softmax2d(x,dim=2)
