...
from tool.dataset import mydataset
import json
...
def train(model, criterion, optimizer, scheduler, dataloader_train,device):
    ...
    for batched_sample in tqdm(dataloader_train):
        ...
        labels = batched_sample['label'].float().to(device)
        ...
        with torch.set_grad_enabled(True):
            ...
            softmax_outputs1 = softmax_outputs[:,1,:,:]
            labels1 = labels1[:, 1,:,:]
            softmax_outputs1 = softmax_outputs1.cpu().data.numpy()
            #print(softmax_outputs.shape)
            softmax_outputs1[softmax_outputs1>=0.5] = 1
            softmax_outputs1[softmax_outputs1 < 0.5] = 0
            flatten_bool_label = (labels1== 1).flatten().astype(np.bool)
            flatten_bool_softmax = softmax_outputs1.flatten().astype(np.bool)
    ...
    #print(stat.get_confusion_matrix())
...
def validate(model, criterion,dataloader_valid, device):
    ...
    for batched_sample in tqdm(dataloader_valid):
        ...
        softmax_outputs1[softmax_outputs1 >= 0.5] = 1
        softmax_outputs1[softmax_outputs1 < 0.5] = 0
        #print(len(softmax_outputs1[softmax_outputs1 ==1]))
        flatten_bool_label = (labels1 == 1).flatten().astype(np.bool)
        flatten_bool_softmax = softmax_outputs1.flatten().astype(np.bool)
        stat.update(flatten_bool_label, flatten_bool_softmax)
        dataset_size += len(batched_sample)
...
def main(args):
    ...
    train_data = mydataset('../CA', '../tool/train.txt')
    ...
    optimizer = optim.SGD(model_params, lr=learning_rate, momentum=momentum,weight_decay=0.01)
    # scheduler = None
